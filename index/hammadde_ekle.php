...
if(isset($_POST['submit'])) {
    ...
    $urunSerkeft = $_POST['urunSerkeft'];
    $urunBarkod = $_POST['urunBarkod'];
    $urunFatura = $_POST['urunFatura'];
    $urunTedarikci = $_POST['urunTedarikci'];
    $alisTarihi = $_POST['alisTarihi'];
    $nakliye=$_POST['nakliyeBedel'];
    $ekBedel=$_POST['ekBedel'];


    // Veritabanına ekleme işlemi

    $sqlHammadde = "INSERT INTO ham_madde (HM_Adi, HM_Stok, HM_Serkeft_Adi, HM_Barkod, HM_Fatura_No, HM_Tedarikci, HM_Alis_Tarihi)";
    $sqlHammadde .= "VALUES ('$urunAdi', '$urunStok', '$urunSerkeft', '$urunBarkod', '$urunFatura', '$urunTedarikci', '$alisTarihi')";
    if($baglan->query($sqlHammadde) === TRUE) {
        // Hammadde başarıyla eklendi, şimdi alış faturası ekleyelim.

        // Alış faturası verilerini hazırlayalım
        $miktar = $urunStok;
        $tedarikci = $urunTedarikci;
        $faturaNo = $urunFatura;
        $barkod = $urunBarkod;
        $tarih = $alisTarihi;
       $nakliyeBedel=$nakliye;
        // Alış faturasını ekleyelim
        $sqlAlisFaturasi = "INSERT INTO alis_faturalari (HM_Adi, miktar, tedarikci, urun_barkod, fatura_no, nakliye_bedeli, ek_bedel, tarih)";
        $sqlAlisFaturasi .= " VALUES ('$urunAdi', '$miktar', '$tedarikci', '$barkod', '$faturaNo', '$nakliyeBedel', '$ekBedel', '$tarih')";

        if($baglan->query($sqlAlisFaturasi) === TRUE) {
            echo "Hammadde başarıyla eklendi ve alış faturası oluşturuldu.";
        } else {
            echo "Hata: Alış faturası eklenirken bir sorun oluştu - " . $baglan->error;
        }
    } else {
        echo "Hata: Hammadde eklenirken bir sorun oluştu - " . $baglan->error;
    }
}
?>
<style>
...
<h2>Ham Madde Ekleme Sayfası</h2>
<form method="POST" action="" style="width: 50%;">
    <div class="form-group">
        ...
        <label for="urunSerkeft">Ürün Serkeft Adı:</label>
        <input type="text" class="form-control" id="urunSerkeft" name="urunSerkeft" required>
    </div>
    <div class="form-group">
        <label for="urunStok">Ürün KG Miktarı:</label>
        <input type="number" class="form-control" id="urunStok" name="urunStok" required>
    </div>
   
    <div class="form-group">
        <label for="urunBarkod">Ürün Barkod:</label>
        <input type="text" class="form-control" id="urunBarkod" name="urunBarkod" required>
    </div>
    <div class="form-group">
        <label for="urunFatura">Fatura No:</label>
        <input type="text" class="form-control" id="urunFatura" name="urunFatura" required>
    </div>
    <div class="form-group">
        <label for="urunTedarikci">Ürün Tedarikci:</label>
        <input type="text" class="form-control" id="urunTedarikci" name="urunTedarikci" required>
    </div>
    <div class="form-group">
        <label for="nakliyeBedel">Nakliye Bedeli:</label>
        <input type="number" class="form-control" id="nakliyeBedel" name="nakliyeBedel" required>
    </div>
    <div class="form-group">
        <label for="ekBedel">Ek Bedel:</label>
        <input type="number" class="form-control" id="ekBedel" name="ekBedel">
    </div>
    <div class="form-group">
