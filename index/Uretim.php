...
$sqlAnaUrunler = "SELECT * FROM urunler";
$resultAnaUrunler = $baglan->query($sqlAnaUrunler);
...
if ($resultHamMadde->num_rows > 0 ) {
  ...
  <div class='form-group'>
    <label for='Ana_Urun'>Mamül:</label>
    <select class='form-control' id='Ana_Urun' name='Ana_Urun' >
  </div>";
        

    while ($row = $resultAnaUrunler->fetch_assoc()) {
        $anaUrunID = $row['Urun_id'];
        $anaUrunAdi = $row['Urun_Adi'];
        echo "<option value='$anaUrunID'>$anaUrunAdi</option>";
    }

    echo "</select><br>";

    // Hammadde seçim ve miktar girişi olarak göster
    echo "<b>Hammadde ve Yarımamüller: </b><br>";
    ...
    while ($row = $resultYariMamuller->fetch_assoc()) {
        ...
        $YMAdi = $row['YM_Adi'];
        echo" <div class='form-group'>
        <input type='checkbox' name='yarimamuller[]' value='$YMID'>
        <label for='ym_adi'> $YMAdi Miktarı:</label>
        <input type='number' class='form-control' id='ym_adi' name='MiktarYarı[$YMID]' step='0.01'><br>
        </div>";
    }
    ...
    echo "hammadde bulunamadı veya yarımamül stok eksiği.";
}
// Yarı mamulleri sorgula
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $anaUrunID = $_POST['Ana_Urun'];
    $barkod = $_POST['Barkod'];
    $partiNo = $_POST['Parti_no'];

    // Hammadde stoklarını kontrol et
    if (isset($_POST['ham_madde'])) {
        foreach ($_POST['ham_madde'] as $selectedHamMadde) {
            if (isset($_POST["MiktarHam"][$selectedHamMadde])) {
                ...
                if ($hamMaddeMiktari > $hamMaddeStok) {
                    echo "Stok yetersiz: " . $row['HM_serkeft_adi'] . "<hr>";
                    exit;
                }
            }
        }
    ...
    if (isset($_POST['yarimamuller'])) {
        foreach ($_POST['yarimamuller'] as $selectedYariMamul) {
            if (isset($_POST["MiktarYarı"][$selectedYariMamul])) {
                $yariMamulMiktari = $_POST["MiktarYarı"][$selectedYariMamul];
                $sqlStokKontrol = "SELECT YM_Stok,YM_Adi FROM yarimamuller WHERE YM_id = $selectedYariMamul";
                $resultStokKontrol = $baglan->query($sqlStokKontrol);
                $row = $resultStokKontrol->fetch_assoc();
                $yariMamulStok = $row['YM_Stok'];

                if ($yariMamulMiktari > $yariMamulStok) {
                    echo "Stok yetersiz: " . $row['YM_Adi'] . "<hr>";
                    exit;
                }
            }
        }
    ...
    $sqlReceteEkle = "INSERT INTO receteler (Recete_no, Tarih, Barkod, Parti_no, Urun_id) VALUES ('$receteNo', '$tarih', '$barkod', '$partiNo', '$anaUrunID')";
    if ($baglan->query($sqlReceteEkle) === TRUE) {
        ...
        if (isset($_POST['ham_madde'])) {
            foreach ($_POST['ham_madde'] as $selectedHamMadde) {
                if (isset($_POST["MiktarHam"][$selectedHamMadde])) {
                    ...
                    $sqlHamMaddeEkle = "INSERT INTO recete_ham_madde (recete_id, HM_id, miktar) VALUES ('$receteID', '$selectedHamMadde', '$hamMaddeMiktari')";
        ...
        if (isset($_POST['yarimamuller'])) {
            foreach ($_POST['yarimamuller'] as $selectedYariMamul) {
                if (isset($_POST["MiktarYarı"][$selectedYariMamul])) {
                    $yariMamulMiktari = $_POST["MiktarYarı"][$selectedYariMamul];
                    $toplamMiktar += $yariMamulMiktari;

                    $sqlStokDusur = "UPDATE yarimamuller SET YM_Stok = YM_Stok - $yariMamulMiktari WHERE YM_id = $selectedYariMamul";
                    if (!$baglan->query($sqlStokDusur)) {
                        echo "Hata: " . $sqlStokDusur . "<br>" . $baglan->error;
                    }

                    $sqlYariMamulEkle = "INSERT INTO recete_ym (Recete_id, YM_id, Miktar) VALUES ('$receteID', '$selectedYariMamul', '$yariMamulMiktari')";
                    if (!$baglan->query($sqlYariMamulEkle)) {
                        echo "Hata: " . $sqlYariMamulEkle . "<br>" . $baglan->error;
                    }
                }
            }
        }

        $sqlStokGuncelle = "UPDATE urunler SET Urun_Stok = Urun_Stok + $toplamMiktar WHERE Urun_id = $anaUrunID";
        if (!$baglan->query($sqlStokGuncelle)) {
            echo "Hata: " . $sqlStokGuncelle . "<br>" . $baglan->error;
        }

        $sqlsatisekle = "INSERT INTO stok_hareketleri (Urun_id, miktar, hareket_Tarihi, hareket_tipi, Recete_id) VALUES ('$anaUrunID', '$toplamMiktar', '$tarih', 'Uretim', '$receteID')";
        if (!$baglan->query($sqlsatisekle)) {
            echo "Hata: " . $sqlsatisekle . "<br>" . $baglan->error;
        }

    } else {
        echo "Hata: " . $sqlReceteEkle . "<br>" . $baglan->error;
    }
}
