...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $hm_barkod= $_POST['HM_Barkod'];
    $miktar = $_POST['Miktar'];
    $iadeKaynagi = $_POST['Iade_Kaynagi'];

    // Ham madde  stoku güncelle
    $sqlStokGuncelle = "UPDATE ham_madde SET HM_Stok = HM_Stok + $miktar WHERE HM_Barkod = $hm_barkod";
    if ($baglan->query($sqlStokGuncelle) !== TRUE) {
        echo "Stok güncelleme hatası: " . $baglan->error;
    }

    // İade bilgilerini veritabanına kaydet
    $sqlIadeKaydet = "INSERT INTO iade (Iade_HamMaddeAdi, Iade_Miktar, Iade_GeldigiYer) VALUES ((SELECT HM_Adi FROM ham_madde WHERE HM_Barkod = '$hm_barkod'), $miktar, '$iadeKaynagi')";
    if ($baglan->query($sqlIadeKaydet) !== TRUE) {
        echo "İade kaydetme hatası: " . $baglan->error;
    }
}
?>
...
<html>
<head>
    <title>Ham Madde İade</title>
</head>
<body>
    <h2>Ham Madde İade</h2>
    <form method="post" action="">
    <div class="form-group">
                                        <label>Ham madde Barkod</label>
                                        <input type="text" class="form-control"  name="HM_Barkod" id="defaultconfig" />
                                    </div>

                                    <div class="form-group">
                                        <label>Miktar</label>
                                        <input type="text"  name="Miktar" class="form-control" id="thresholdconfig" />
                                    </div>

                                    <div class="form-group">
                                        <label>İade Kaynağı</label>
                                        <input type="text" class="form-control"  name="Iade_Kaynagi" id="placement" />
                                    </div>
