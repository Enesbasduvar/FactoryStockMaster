...
<!-- Left Sidebar End -->
        ...
        <div class="card">
                                <div class="card-body">
                                    ...
                                    <h4 class="card-title">yarı mamül kayıt</h4>
                                

                                    <div class="form-group">
                                        <label>yarimamül adı</label>
                                        <input type="text" class="form-control"  name="YM_Adi" id="defaultconfig" />
                                    </div>

                                    <div class="form-group">
                                        <label>stok bilgisi</label>
                                        <input type="text"  name="YM_Stok" class="form-control" id="thresholdconfig" />
                                    </div>

                                    <div class="form-group">
                                        <label>Barkod</label>
                                        <input type="text" class="form-control"  name="YM_Barkod" id="placement" />
                                    </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    include("Baglan.php"); 
        // Ana ürün ekleme
        $urunAdi = $_POST['YM_Adi'];
        $stokAdeti = $_POST['YM_Stok'];
        $barkod = $_POST['YM_Barkod'];
        $hmBilgi = $_POST['Hammadde_Bilgileri'] ;

        // Mevcut ürünleri kontrol et
        $sqlUrunKontrol = "SELECT * FROM yarimamuller WHERE YM_Barkod= '$barkod'";
        $result = $baglan->query($sqlUrunKontrol);

        if ($result->num_rows > 0) {
            // Ürün zaten var, stok miktarını artır
            $row = $result->fetch_assoc();
            $stokAdeti += $row["YM_Stok"];

            $sqlUrunGuncelle = "UPDATE yarimamuller SET YM_Stok = $stokAdeti WHERE YM_Barkod = '$barkod'";
            if ($baglan->query($sqlUrunGuncelle) !== TRUE) {
                echo "Ürün güncelleme hatası: " . $baglan->error;
            }
        } else {
            // Yeni ürün ekle
            $sqlAnaUrunEkle = "INSERT INTO yarimamuller (YM_Adi, YM_Stok, YM_Barkod, Hammaddeler)  VALUES ('$urunAdi', $stokAdeti, '$barkod', '$hmBilgi')";
            if ($baglan->query($sqlAnaUrunEkle) !== TRUE) {
                echo "ürün ekleme hatası: " . $baglan->error;
            }
        }
