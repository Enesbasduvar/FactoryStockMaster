...
if (isset($_GET['Recete_no']) && !empty($_GET['Recete_no'])) {
    ...
    if ($resultRecete->num_rows > 0) {
        ...
        $sqlAnaUrun = "SELECT YM_Adi FROM yarimamuller WHERE YM_id = $YM_ID";
        $resultAnaUrun = $baglan->query($sqlAnaUrun);
        $anaUrunAdi = ($resultAnaUrun->num_rows > 0) ? $resultAnaUrun->fetch_assoc()['YM_Adi'] : 'Bilinmiyor';
            ...
            echo "<div style='border: 1px solid black; padding: 10px; width: 500px;'>
                    <b>Tarih:</b> $tarih<br>
                    <b>Reçete No:</b> $receteNo<br>
                    <b>Yarı Mamül:</b> $anaUrunAdi<br>
                    <b>Barkod:</b> $barkod<br>
            ...
            $toplamMiktarYan = 0; // Yan ürünler için toplam miktar

           

            // Ham maddeleri sorgula
            $sqlHamMaddeler = "SELECT * FROM ham_madde INNER JOIN ym_hm_recete ON ham_madde.HM_id = ym_hm_recete.HM_id WHERE ym_hm_recete.recete_ym_id = $receteID";
...
if ($resultHamMaddeler->num_rows > 0) {
    while ($rowHamMadde = $resultHamMaddeler->fetch_assoc()) {
        ...
        $toplamMiktarYan += $hamMaddeMiktari;
    }
}
...
while ($rowHamMadde = $resultHamMaddeler->fetch_assoc()) {
    $hamMaddeAdi = $rowHamMadde['HM_serkeft_adi'];
    $hamMaddeMiktari = $rowHamMadde['Miktar'];
    $yuzdeHam = round(($hamMaddeMiktari / $toplamMiktarYan) * 100, 2);
    echo "<tr>
            <td>$hamMaddeAdi</td>
            ...
            echo "<tr>
                    <td><b>Toplam Miktar:</b></td>
                    <td><b>$toplamMiktarYan</b></td>
                    <td><b>100%</b></td>
                  </tr>
                  </table>";
