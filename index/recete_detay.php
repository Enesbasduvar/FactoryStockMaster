...
<body>
    ...
    <?php
    if (isset($_GET['Recete_no']) && !empty($_GET['Recete_no'])) {
        $receteNo = $_GET['Recete_no'];

        // Reçeteyi sorgula
        $sqlRecete = "SELECT * FROM receteler WHERE Recete_no = $receteNo";
        $resultRecete = $baglan->query($sqlRecete);

        if ($resultRecete->num_rows > 0) {
            $row = $resultRecete->fetch_assoc();
            $receteID = $row['Recete_id'];
            $tarih = $row['Tarih'];
            $anaUrunID = $row['Urun_id'];
            $barkod = $row['Barkod'];

            // Ana ürünü sorgula
            $sqlAnaUrun = "SELECT Urun_Adi FROM urunler WHERE Urun_id = $anaUrunID";
            $resultAnaUrun = $baglan->query($sqlAnaUrun);
            $anaUrunAdi = ($resultAnaUrun->num_rows > 0) ? $resultAnaUrun->fetch_assoc()['Urun_Adi'] : 'Bilinmiyor';

            // Ham maddeleri sorgula
            $sqlHamMaddeler = "SELECT * FROM ham_madde INNER JOIN recete_ham_madde ON ham_madde.HM_id = recete_ham_madde.HM_id WHERE recete_ham_madde.recete_id = $receteID";
            $resultHamMaddeler = $baglan->query($sqlHamMaddeler);

            // Yarı mamulleri sorgula
            $sqlYariMamuller = "SELECT * FROM yarimamuller INNER JOIN recete_ym ON yarimamuller.YM_id = recete_ym.YM_id WHERE recete_ym.Recete_id = $receteID";
            $resultYariMamuller = $baglan->query($sqlYariMamuller);
    ?>
            <div class="container">
                <h2>Reçete Detayı</h2>
                <b>Tarih:</b> <?php echo $tarih; ?><br>
                <b>Reçete No:</b> <?php echo $receteNo; ?><br>
                <b>Barkod:</b> <?php echo $barkod; ?><br>
                <b>Ana Ürün:</b> <?php echo $anaUrunAdi; ?><br>
                ...
                <table>
                    ...
                    <tbody>
                        <?php
                        // Ham maddeler ve yarı mamuller için toplam miktarı hesapla
                        $toplamMiktar = 0;
                        if ($resultHamMaddeler->num_rows > 0) {
                            while ($rowHamMadde = $resultHamMaddeler->fetch_assoc()) {
                                $hamMaddeMiktari = $rowHamMadde['miktar'];
                                $toplamMiktar += $hamMaddeMiktari;
                            }
                        }
                        if ($resultYariMamuller->num_rows > 0) {
                            while ($rowYariMamul = $resultYariMamuller->fetch_assoc()) {
                                $yariMamulMiktari = $rowYariMamul['Miktar'];
                                $toplamMiktar += $yariMamulMiktari;
                            }
                        }

                        // Ham maddeler için yüzdeleri hesapla
                        if ($resultHamMaddeler->num_rows > 0) {
                            mysqli_data_seek($resultHamMaddeler, 0);
                            while ($rowHamMadde = $resultHamMaddeler->fetch_assoc()) {
                                $hamMaddeAdi = $rowHamMadde['HM_serkeft_adi'];
                                $hamMaddeMiktari = $rowHamMadde['miktar'];
                                $yuzdeHam = round(($hamMaddeMiktari / $toplamMiktar) * 100, 2);
                                echo "<tr>
                                        <td>$hamMaddeAdi</td>
                                        <td>$hamMaddeMiktari</td>
                                        <td>$yuzdeHam%</td>
                                      </tr>";
                            }
                        }

                        // Yarı mamuller için yüzdeleri hesapla
                        if ($resultYariMamuller->num_rows > 0) {
                            mysqli_data_seek($resultYariMamuller, 0);
                            while ($rowYariMamul = $resultYariMamuller->fetch_assoc()) {
                                $yariMamulAdi = $rowYariMamul['YM_Adi'];
                                $yariMamulMiktari = $rowYariMamul['Miktar'];
                                $yuzdeYan = round(($yariMamulMiktari / $toplamMiktar) * 100, 2);
                                echo "<tr>
                                        <td>$yariMamulAdi</td>
                                        <td>$yariMamulMiktari</td>
                                        <td>$yuzdeYan%</td>
                                      </tr>";
                            }
                        }
