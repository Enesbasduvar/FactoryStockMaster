...
<body>
    ...
    include("Baglan.php");
    // Ana ürünleri sorgula ve listele
    echo "<h3>Ana Ürünler</h3>";
    $sqlAnaUrunler = "SELECT * FROM urunler";
    $resultAnaUrunler = $baglan->query($sqlAnaUrunler);

    if ($resultAnaUrunler->num_rows > 0) {
        ...
        while ($row = $resultAnaUrunler->fetch_assoc()) {
            ...
            <td>". str_replace(',', ', ', $row['Hammaddeler']) ."</td>
            <td>". str_replace(',', ', ', $row['Yarimamuller']) ."</td>
            <td><a href='Guncelle.php?id={$row['Urun_id']}&tur=ana_urun'>Güncelle</a></td>
            <td><a href='?delete={$row['Urun_id']}&tur=ana_urun'>Sil</a></td>           
            </tr>";
        }
        echo "</table>";
    } else {
        echo "Ana ürün bulunamadı.";
    }?>
                        ...
                        <?php
      // Verileri Silme
      function deleteProduct($baglan, $table, $idColumn, $id) {
    ...
    if (isset($_GET['delete'])) {
        $deleteID = $_GET['delete'];
            ...
            deleteProduct($baglan, 'urunler', 'Urun_id', $deleteID);
